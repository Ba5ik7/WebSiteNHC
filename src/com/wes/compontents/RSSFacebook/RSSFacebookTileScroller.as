package com.wes.compontents.RSSFacebook
{
	...
	public class RSSFacebookTileScroller extends Sprite
	{
		...
		public function RSSFacebookTileScroller(_arrayW : Array, _widthW : int = 870, _heightW : int = 400)
		{
			super();
			_array = _arrayW;
			_width = _widthW;
		...
		private function buildCon():void
		{
			...
			addChild(_con);
			var i : int = 0;
			for each (var _tile : FacebookRSSItemObj in _array) {
				var tile : RSSFacebookTile = new RSSFacebookTile(_tile);
				tile.y = i;
				_con.addChild(tile);
				i += tile.height + 10;
			}
